...
/// **第二行**（`上下文 N%` + `子代理 N`）的字号。用户 2026-09-15 裁定："**缩小 50%**"。
///
/// （旧注释叫它"第三行" —— 那是每会话**三行**那个版本的叫法，三行布局已被用户否掉，
/// 现在一行会话只有两行。同一个词残留在别处会让人以为界面上还有第三行。）
///
/// 写成 `STATE_SIZE / 2.0` 而不是 `7.5`：这个值**定义为状态词的一半**，状态词将来若改，
/// 它跟着走；写死数字会让两者悄悄脱钩。
const CONTEXT_SIZE: f32 = STATE_SIZE / 2.0;
...
/// （galley 原点差；同一处的视觉空隙是 50），将来谁动了行高、`rule()` 或 `item_spacing`
/// 都会让它失败，而不是界面悄悄变回去。
///
...
/// 试过 15pt，结果这一行的行高从 8pt 涨到 16pt —— 连带要重标 `ROW_LINE_GAP` /
/// `ROW_BOTTOM_GAP` / `EXTRA_ROW_GAP` 三个常量（实测"第一行→第二行"会从 14 变 18、
/// 会话间距从 58 变 62），而且每个会话都变高、窗口能装的会话数变少。
/// 用同一号字则**一分钱不花**：行高不变、四段间距一个都不用动。
...
/// 补充。**真机上量过**：默认 320pt 宽、内容区 298pt 时，第一行的固定开销
/// （`▍` 10.2 + `等待确认` 60 + `01:12` 52 + 3 个间隙 24 ≈ 146）留给会名约 **151pt**；
/// 而 `上下文 68%` = 87.25pt、`子代理 2` ≈ 62pt，再加 2 个间隙 16pt = 165pt ——
/// 塞进第一行的话会名额度会变成 **−14pt**（会名直接消失）。所以分行是唯一可行解。
/// 代价是每行高度翻倍，但**是常数**（固定两行），不随子代理数量增长。
///
...
fn draw_row(ui: &mut egui::Ui, row: &view::Row, ledger: &usage::LedgerView, bar_color: egui::Color32, cfg: &config::Config) {
    ...
    // 第二行同理：行高取**这一行里最高的那个字块**。token 那一格（15pt）比
    // `上下文 N%`（7.5pt）高，只按 7.5pt 算行高的话，超出的 8pt 会**静默压到下一行上**
    // （`row_line` 的推进量恒等于传进去的行高，内容撑不大它）—— 实测过：
    // 少了这一句，"会话间距 58pt" 与"第一行→第二行 14pt"两条断言立刻红。
    ...
    // 实测（2026-09-15）：最坏情况 `上下文 100% + 子代理 12` 右缘 105.75pt，
    // 200pt 窗口的内容右界 189pt，缩进 18pt 后仍空 65pt。
    // 现在保持顶格是**用户的取舍**，不是宽度不够 —— 别再拿"放不下"当理由。
    row_line(ui, ctx_row_h, |ui| {
        ...
        // 第二行左半本来是空的（`上下文 N%` 只有 7.5pt 宽），放这里**一分钱不花**。
        ui.label(
...
mod tests {
    ...
    #[test]
    fn the_context_line_is_half_the_size_of_the_state_word() {
        // 用户 2026-09-15 裁定："上下文和对应的占比**缩小 50%**"。
        ...
        assert!(
            (ratio - 0.5).abs() < 0.06,
            "占比行字高 {ctx} / 状态词字高 {state} = {ratio}，应约为 0.5"
        );
    }
